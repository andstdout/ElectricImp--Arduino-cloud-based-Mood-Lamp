...
//---------------------CUSTOM-CHANGES-by-Andreas-Pagel-------------------------
//I've changed the first element at dim_curve from 0 to 1 because of flickering
//There is a bug at "base = ((255 - sat) * val)>>8;" - line, it must be an unsigned value:
//base = unsigned((255 - sat) * val)>>8;
...
void HsbConverter(int hue, int brightness, int saturation)  { 
  int rgb_colors[3];
  getRGB(hue,saturation,brightness,rgb_colors);   // converts HSB to RGB
  setLedColor(rgb_colors[0],rgb_colors[1],rgb_colors[2]); //control the leds
